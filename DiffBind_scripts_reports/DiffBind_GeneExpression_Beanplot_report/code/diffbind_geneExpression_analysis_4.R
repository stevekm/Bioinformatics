...
# # only get entries that are 3Kbp or less from the feature. distancetoFeature
# keep only entries with logfold change >1.5x; log2 = 0.58

diffbind_df_intersect_min <- subset(diffbind_df_intersect, abs(distancetoFeature)<=3000) # & abs(Fold)>=1.5
...
diffbind_df_intersect_min <- diffbind_df_intersect_min[,! colnames(diffbind_df_intersect_min) %in% diffbind_cols_to_remove]
...
# diff_gene_expr_merged_subset <- subset(diff_gene_expr_merged, abs(diff_peak_log_ratio) >= 0.58)
diff_gene_expr_merged_subset <- diff_gene_expr_merged
# ~~~~~~~~~~~~~~~ #
...
# base violin plot
p <- ggplot(diff_gene_expr_merged_subset, aes(x=sample, y=diff_peak_log_ratio, group=sample)) + geom_violin(trim=FALSE,fill="darkorchid4",color=NA,scale="width")
...
# # hold off on these, the strip chart especially does not come out well but this code might be useful
...
tt <- ggplot(data=diff_gene_expr_merged_subset)
# add up genes
tt <- tt + geom_density(data=subset(diff_gene_expr_merged_subset,gene_expression_status == unique(diff_gene_expr_merged_subset[["gene_expression_status"]])[2]), aes(x=diff_peak_log_ratio,y=..density..,fill=gene_expression_status), trim=F,alpha=0.4,adjust = dens_adjust,color=NA)
# add down genes
tt <- tt + geom_density(data=subset(diff_gene_expr_merged_subset,gene_expression_status == unique(diff_gene_expr_merged_subset[["gene_expression_status"]])[3]), aes(x=diff_peak_log_ratio,y=..density..,fill=gene_expression_status), trim=F,alpha=0.4,adjust = dens_adjust,color=NA)
# add all genes
tt <- tt + geom_density(data=diff_gene_expr_merged_subset, aes(x=diff_peak_log_ratio,y=-..density..,fill="ALL", trim=F),alpha=1,adjust = dens_adjust,color=NA)
# rotate axis
tt <- tt + coord_flip() 
# add x labels
...
# > unique(diff_gene_expr_merged_subset[["gene_expression_status"]])
# [1] NA     "DOWN" "UP"  
save.image(file=paste0(main_outdir,"/data.Rdata"),compress = TRUE)

# dt = data.table(df)
# dt[,list(player_count = length(unique(soccer_player))),by=country]
# diff_gene_expr_merged_subset_dt <- data.table(diff_gene_expr_merged_subset)

# diff_gene_expr_merged_subset_dt[,list(count = length(unique(gene))),by=list(sample,gene_expression_status)]

plot_signif_pvalue <- function(df, y_colname, x_colname, x_group, p_value_cutoff=0.05){
  ...
  for(i in seq_along(unique(as.character(df[[x_group]])))){
    ...
    tmp_pvalue <- t.test(get(y_colname)~get(x_colname),data = subset(df, get(x_group) == tmp_ID))["p.value"]
    
    if(tmp_pvalue < p_value_cutoff){
      print(points(x = i,y = y_max - 3,pch ='*', col ='red',cex=2))
      print(text(x = i,y = y_max - 2,labels = paste0('p = ',format(tmp_pvalue, digits = 2)),cex=0.7))
    }
  }
}

if(nrow(unique(subset(diff_gene_expr_merged_subset, gene_expression_status=="UP")["gene"]))>=50 &&
   nrow(unique(subset(diff_gene_expr_merged_subset, gene_expression_status=="DOWN")["gene"]))>=50){
  # base R beanplot
  ...
  beanplot(diff_peak_log_ratio~gene_expression_status*sample,
           ...
           data=diff_gene_expr_merged_subset,
           what=c(0,1,1,0),
           # what=c(0,1,1,1),maxstripline = 0, # ll = 0.04,varwidth = TRUE,
           border = NA,
  ...
  plot_signif_pvalue(df = diff_gene_expr_merged_subset,y_colname = 'diff_peak_log_ratio',x_colname = 'gene_expression_status', x_group = 'sample')
  # hold off on this strip chart overlay:
  # stripchart(diff_peak_log_ratio~gene_expression_status*sample,data = diff_gene_expr_merged_subset,
