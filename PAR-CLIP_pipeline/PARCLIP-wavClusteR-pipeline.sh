...
set -x
##
## USAGE: PARCLIP-wavClusteR-pipeline.sh /path/to/outdir /path/to/input_file_R1.fastq.gz /path/to/input_file_R2.fastq.gz <sampleID> <ref_genome> /path/to/ref_genome  
...
# ~~~~~~ # ~~~~~~ # ~~~~~~ run commands ~~~~~~ # ~~~~~~ # ~~~~~~ # ~~~~~~ 
# our files are fastq.gz, so need to unzip them for this... ;_;
...
Rscript --slave --no-save --no-restore - "${SAMPLEID}" "${SAMPLEID}.bam" <<EOFB
  ...
  # this step not working for some reason, troubleshoot this some more
