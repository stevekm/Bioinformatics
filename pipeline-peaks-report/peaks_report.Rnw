...
Project_ID<-latexTranslate("SmithLab-ChIPSeq_2016-12-31 Peaks per Sample per Peak Calling setting") # for the title slide
Report_author<-latexTranslate("Stephen Kelly")
PI_name<-latexTranslate("Dr. X")
Report_author_email<-latexTranslate("stephen.kelly@nyumc.org")

ProjDir<-"/ifs/home/kellys04/projects/SmithLab-ChIPSeq_2016-12-31/analysis_dir/project_notes/peaks-per-sample_report_3"
pipelineDir<-paste0(ProjDir,"/output")
...
SampleSheet_file<-"/ifs/data/smithlab/SmithLab-ChIPSeq_2016-12-31/inputs/sample-sheet.tsv"
...
cat("\\begin{frame}[allowframebreaks]{", latexTranslate(" Analysis Sample Sheet"), " }\n", sep="")
  cat("\\small{",sep = "\n")
  # cat(latexTranslate(tmpAlignStats),sep = "\n\n")   
  print(xtable(x = PipelineSampleSheet[,grep("^R|^ref",colnames(PipelineSampleSheet),invert = T)]),size="scriptsize",tabular.environment="longtable",floating=FALSE) 
...
for(i in seq_along(sampleDirs)){
  ...
  tmpSamplePDF_files<-dir(tmpSampleDir,pattern = "dual_ggbarplots.pdf") #dual_ggbarplots.pdf
  ...
  # PRINT THE STATS
  # # get the stats file
  tmpAlignStatsFile<-dir(tmpSampleDir,pattern = "peak_align_stats.tsv",full.names = TRUE)
  # read in the file
  ...
  # re order the sheet based on the second column
  tmpAlignStats <- tmpAlignStats[order(tmpAlignStats[2]),]
  rownames(tmpAlignStats) <- seq(1:nrow(tmpAlignStats))
  # print it in a new frame
  cat("\\subsubsection{", latexTranslate("Peak"), "}\n", sep="")
