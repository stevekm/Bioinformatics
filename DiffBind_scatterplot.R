...
DiffBind_scatter_plot_top_protein_coding_promoters <- function(diff_df, signif_p_value=0.05, fold_change_value=1, plot_colors=c("grey","red","orange","blue", "green"), top_gene_number=25){
    ...
    diff_df_min_signiff_UpFold <- diff_df_min_signiff_protein[with(diff_df_min_signiff_protein, order(Fold, p.value)), c(sampleID_1,sampleID_2,"Fold","p.value","external_gene_name")][1:top_gene_number,]
    
    diff_df_min_signiff_DnFold <- diff_df_min_signiff_protein[with(diff_df_min_signiff_protein, order(-Fold, p.value)), c(sampleID_1,sampleID_2,"Fold","p.value","external_gene_name")][1:top_gene_number,]
    
    
    library("calibrate")
    # plot_lim <- max(na.rm = TRUE,abs(c(na.omit(diff_df_min_signiff_protein[[sampleID_2]]), na.omit(diff_df_min_signiff_protein[[sampleID_1]])))) + 2
    plot_lim <- 10
    print(plot_lim)
    print(plot_lim)
    # base plot, all points
    plot(x = diff_df_min_signiff_protein[[sampleID_2]],
         y=diff_df_min_signiff_protein[[sampleID_1]],
         xlab = sampleID_2,
         ylab = sampleID_1,
         col="grey",
         xlim = c(-1,plot_lim),
    ...
    points(x = diff_df_min_signiff_UpFold[[sampleID_2]],
           y = diff_df_min_signiff_UpFold[[sampleID_1]],
           col='blue')
    
    textxy(diff_df_min_signiff_UpFold[[sampleID_2]],
           diff_df_min_signiff_UpFold[[sampleID_1]],
           labs=diff_df_min_signiff_UpFold[["external_gene_name"]])
    
    points(x = diff_df_min_signiff_DnFold[[sampleID_2]],
           y = diff_df_min_signiff_DnFold[[sampleID_1]],
           col='blue')
    
    textxy(diff_df_min_signiff_DnFold[[sampleID_2]],
           diff_df_min_signiff_DnFold[[sampleID_1]],
           labs=diff_df_min_signiff_DnFold[["external_gene_name"]])
...
for(i in c(1,3)){
    ...
    print(paste0(out_dir,"/scatterPlot_",names(sample_files[i]),"_TopPromoter_proteinCoding.pdf"))
    pdf(file = paste0(out_dir,"/scatterPlot_",names(sample_files[i]),"_TopPromoter_proteinCoding.pdf"),height = 8,width = 8)
    print(DiffBind_scatter_plot_top_protein_coding_promoters(diff_df))
    dev.off()
