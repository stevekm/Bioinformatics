...
ReadsTable<-structure(list(Total.Reads = c(43743601.2, 37888316.7, 44664123.66, 39804329.76, 36708833.46, 49666748.82, 47457348.9, 48866464.92), 
                           ...
                           Deduplicated.alignments = c(32004797.76, 26982520.5, 29728794.42, 28707722.94, 26717704.26, 25559734.98, 22385229.36, 34029148.92)), 
                      .Names = c("Total.Reads", "Aligned.Reads", "Deduplicated.alignments"), 
...
ReadsTable$PercentDedup=signif(c(ReadsTable$Deduplicated.alignments / ReadsTable$Total.Reads)*100,digits = 4 )
...
ReadsTable$Duplicated=c(ReadsTable$Aligned.Reads - ReadsTable$Deduplicated.alignments)
...
Dup_Raw_Reads_Matrix<-t(as.matrix( ReadsTable[,which(colnames(ReadsTable) %in% c("Deduplicated.alignments","Duplicated","UnalignedReads")) ] ))
# # divid the number of reads by 1million
Dup_Raw_Reads_Matrix<-signif(Dup_Raw_Reads_Matrix/1000000,digits = 3)
